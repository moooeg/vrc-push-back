...
def double_parking():
    ...
    while True:
        ...
        if controller_1.buttonY.pressing():
            ...
            while not (40 < park_distance.object_distance(MM) < 60):
...
def auto_skill():
    if 1:
        ...
        wait(1000, MSEC)
        drivetrain_forward(0.68, -0.68, False, 100)
        drivetrain_forward(5.15, 5.15, False, 100)
        drivetrain_forward(0.45, -0.45, False, 100)
        drivetrain_forward(-3, -3, False, 100, 800)
        wait(0.2, SECONDS)
        ...
        wait(2500, MSEC)
        intake1.spin(REVERSE, 10, PERCENT)
        drivetrain_forward(1, 1, time_out=1600, speed=50)
        wait(0.5, SECONDS)
        drivetrain_forward(-4, -4, time_out=800)
        intake1.spin(FORWARD, 100, PERCENT)
        wait(2.5, SECONDS)
        intake3.stop()
        intake1.stop()
        drivetrain_forward(4.3, 4.3)
        drivetrain_forward(0.5, -0.5)
        intake1.spin(FORWARD)
        intake3.spin(FORWARD)
        match_load.set(True)
...
def user_control():
    ...
    while True:
        if controller_1.buttonY.pressing():
            ...
            match_load.set(True)
            intake1.set_velocity(50, PERCENT)
            while not (35 < park_distance.object_distance(MM) < 50):
                intake1.spin(REVERSE)
                if controller_1.buttonB.pressing():
                    double_park.set(False)
                    break
